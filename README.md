...
1. 기본 번역은 API 키 없이 사용합니다(인터넷 필요). 설정의 **번역 엔진 · 언어** 카드에서 `무료 (비공식 웹)` / `Google 번역 API` / `DeepL API` / `Papago NMT` 중 하나를 고르고, 원문·번역 언어를 정합니다. 무료 엔진은 키가 필요 없고, 나머지는 아래 API 키 칸을 채워야 동작합니다(Papago는 `ClientID:ClientSecret` 형식). 키가 없거나 잘못되면 일반 연결 오류가 아니라 그 이유를 알려 줍니다.
...
설정은 `%LOCALAPPDATA%\hoonsoo\settings.json`. 원문·스크린샷을 로그에 기록하지 않습니다. 화면 이미지는 메모리에서만 로컬 OCR에 사용하고 네트워크로 전송하지 않습니다. **번역할 일반 텍스트는 선택한 엔진의 서비스로 전송됩니다** — 기본값인 무료 엔진은 키 없는 비공식 Google 웹 엔드포인트(실패 시 MyMemory 대체)를 쓰고, `Google 번역 API`·`DeepL API`·`Papago NMT`를 고르면 그 서비스(Google Cloud / DeepL / Naver Cloud)로 전송됩니다. 비공식 엔드포인트는 서비스 변경/차단 시 번역이 실패할 수 있어, 차단되면 2분간 대체 서비스를 씁니다. 코드 보호 구간은 전송하지 않습니다. 번역 외의 목적으로 원문을 전송하는 경로는 없습니다. API 키를 입력하면 `settings.json`에 **평문**으로 저장되므로 자격 증명 파일은 만들지 않지만, 그 파일을 공유하거나 백업할 때는 주의하세요. 전체 화면 AR 자막은 무료 엔드포인트만 사용하며, 설정한 원문·번역 언어를 그대로 따릅니다.
